...
/* Used only as the fallback when no variant is selected */
...
function Gallery({ images, name }) {
  ...
  if (!imgs.length) return <div className={styles.galleryEmpty}>No image available</div>;
    ...
    <div className={styles.gallery}>
      {imgs.length > 1 && (
        <div className={styles.thumbs}>
          {imgs.map((src, i) => (
            <button key={i} className={`${styles.thumb} ${i === active ? styles.thumbOn : ""}`}
              onClick={() => setActive(i)}>
              <img src={src} alt={`View ${i + 1}`} onError={e => { e.currentTarget.style.opacity = "0.2"; }}/>
            </button>
          ))}
        </div>
      )}
      <div className={styles.mainImgWrap}
        ...
        <img ref={imgRef} src={imgs[active]} alt={name} className={styles.mainImg}
          style={zoomed ? { transformOrigin:`${zoomPos.x}% ${zoomPos.y}%`, transform:"scale(1.9)", cursor:"crosshair" } : {}}
        ...
        {imgs.length > 1 && (
          <div className={styles.imgCounter}>
            {String(active + 1).padStart(2,"0")}<span className={styles.counterSep}>/</span>{String(imgs.length).padStart(2,"0")}
          </div>
        )}
        {!zoomed && (
      ...
      {imgs.length > 1 && (
        <div className={styles.imgDots}>
          {imgs.map((_, i) => (
            <button key={i} className={`${styles.imgDot} ${i === active ? styles.imgDotOn : ""}`} onClick={() => setActive(i)}/>
          ))}
        </div>
      )}
    </div>
  ...
  const handleBuyNow = () => {
    ...
    navigate("/checkout", {
      state: {
        buyNow: {
          product,
          selectedVariant: selVariant || { size: selSize, color: selColor, price: finalPrice, discountedPrice: finalPrice },
          quantity: qty,
        },
      },
    });
    ...
    <div className={styles.page}>
      ...
      <nav className={styles.crumbs}>
        ...
        {product.category?.slug && product.category?.name ? (
          <>
            <Link to={`/collections/${product.category.slug}`} className={styles.crumb}>{product.category.name}</Link>
            <span className={styles.crumbSep}>◆</span>
          </>
        ) : null}
      ...
      <div className={styles.grid}>
        ...
        <div className={styles.infoCol}>
          ...
          <div className={styles.priceBlock}>
            ...
            {hasDis && origPrice && (
              <span className={styles.origPrice}>{fmtPrice(origPrice)}</span>
            )}
            {hasDis && origPrice && (
            ...
            <div className={styles.selector}>
              ...
              <div className={styles.sizeRow}>
                {sizeOpts.map(({ size, stock }) => {
                  ...
                  return (
                    <button key={size}
                      className={`${styles.sizeBtn} ${selSize === size ? styles.sizeBtnOn : ""} ${stock === 0 ? styles.sizeBtnOut : ""}`}
                      onClick={() => stock > 0 && setSelSize(size)}
                      disabled={stock === 0}
                      title={sp ? fmtPrice(sp) : size}>
                      {size}
                    </button>
                  );
                })}
          ...
          <div className={styles.qtyCtaRow}>
            ...
            <button className={`${styles.wishBtn} ${wishlisted ? styles.wishOn : ""}`}
          ...
          <div className={styles.actionBtns}>
            <button className={`${styles.addBtn} ${adding ? styles.addBtnLoading : ""}`}
              ...
              {(product.variants || []).length > 0 && (
                <div className={styles.varTable}>
                  ...
                  <div className={styles.varGrid}>
                    {["Colour","Size","SKU","Price","Stock"].map(h => <span key={h} className={styles.varTH}>{h}</span>)}
                    {(product.variants || []).filter(v => v.isActive !== false).map((v, i) => ([
                      <span key={`c${i}`} className={styles.varTD}>
                        <span className={styles.varDot} style={{ background: v.colorCode || "#888" }}/>{v.color}
                      </span>,
              ...
              <div className={styles.sizeTable}>
                ...
                {[["XS","32–33","24–25","34–35","38"],["S","34–35","26–27","36–37","39"],
                  ...
                  <div key={row[0]} className={`${styles.sizeTRow} ${selSize===row[0]?styles.sizeTRowOn:""}`}>
                    {row.map((cell,ci)=><span key={ci} className={ci===0?styles.sizeTSize:""}>{cell}</span>)}
                  </div>
                ))}
              </div>
            </div>
      ...
      {related.length > 0 && (
        <div className={styles.related}>
          <div className={styles.relHead}>
            <div className={styles.relTitle}><span className={styles.relEye}>◆</span>You may also like</div>
            {product.category?.slug && product.category?.name && (
